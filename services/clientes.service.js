...
class ClientesService {
  ...
  async find(negocioId) {
    const negocio = await models.Negocio.findByPk(negocioId, {
      include: [{
        ...
        as: 'clientes', // Asegúrate de que esta coincida con la definición de tu asociación en el modelo Negocio
        include: [
          {
  ...
  async findOne(negocioId, clienteId) {
    ...
    if (cliente.dataValues.negocioId != negocioId) {
      throw boom.notFound('el cliente no pertenece al negocio');
    }
    return cliente;
  }
  ...
  async sumarSaldo(negocioId, clienteId, monto) {
    ...
    if (!rta) {
      throw boom.notFound('No se pudo sumar saldo al cliente: ' + cliente);
    }
  }
  async restarSaldo(negocioId, clienteId, monto) {
    ...
    const rta = await cliente.update({ saldo: cliente.saldo - monto });
    if (!rta) {
      throw boom.notFound('No se pudo restar saldo al cliente: ' + cliente);
    }
  }
}
